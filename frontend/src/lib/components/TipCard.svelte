			...
			{:else if locked}
				...
				{#if existing}
					...
					{#if played && myFriendEntry?.components}
            			...
            			{@const myEtGTotal = mc.koEtExactHome + mc.koEtExactAway}
						<div class="ybreakdown">
							{#if isKO}
								...
								{/if}
								{#if mc.koFtTendency > 0}
								  <span class="ycomp fc-tendency">W +{mc.koFtTendency}</span>
								{:else}
								  <span class="ycomp fc-miss">FT W</span>
								{/if}
								...
								{/if}
								{#if myEtPredicted}
									 {#if mc.koEtTendency > 0}
                  						<span class="ycomp fc-tendency">ET W +{mc.koEtTendency}</span>
									{:else}
										<span class="ycomp fc-miss">ET W</span>
									{/if}
									{#if mc.koEtGoalDiff > 0}
										<span class="ycomp fc-diff">ET Δ +{mc.koEtGoalDiff}</span>
									{:else}
										<span class="ycomp fc-miss">ET Δ</span>
									{/if}
									{#if myEtGTotal > 0}
										<span class="ycomp fc-goals">ET G +{myEtGTotal}</span>
									{:else}
										<span class="ycomp fc-miss">ET G</span>
									{/if}
									{#if mc.koEtExact > 0}
										<span class="ycomp fc-exact">ET = +{mc.koEtExact}</span>
									{:else}
									...
									<tr class:fme={f.isMe}>
										...
										<td class="ftip">
											...
											{#if c}
                        						...
                        						{@const etGTotal = c.koEtExactHome + c.koEtExactAway}
												<span class="fcomps">
													{#if isKO}
														<!-- Advancer -->
														{#if c.koAdvancer > 0}
														...
														<!-- FT outcome -->
							                            {#if c.koFtTendency > 0}
							                              <span class="fcomp fc-tendency">W +{c.koFtTendency}</span>
							                            {:else}
							                              <span class="fcomp fc-miss">W</span>
							                            {/if}
														<!-- FT goal diff -->
														{#if c.koFtGoalDiff > 0}
														...
														{#if etPredicted}
															{#if c.koEtTendency > 0}
                               									<span class="fcomp fc-tendency">ET W +{c.koEtTendency}</span>
															{:else}
																<span class="fcomp fc-miss">ET W</span>
															{/if}
															{#if c.koEtGoalDiff > 0}
								                                <span class="fcomp fc-diff">ET Δ +{c.koEtGoalDiff}</span>
															  {:else}
																<span class="fcomp fc-miss">ET Δ</span>
															  {/if}
															  {#if etGTotal > 0}
																<span class="fcomp fc-goals">ET G +{etGTotal}</span>
															  {:else}
																<span class="fcomp fc-miss">ET G</span>
															  {/if}
															  {#if c.koEtExact > 0}
																<span class="fcomp fc-exact">ET = +{c.koEtExact}</span>
															  {:else}
