<script lang="ts">
				...
				await Promise.all(
					r.leagues.map(async (l) => {
						try {
							...
							if (res.goldenBoot) goldenBoots[l.id] = res.goldenBoot;
						} catch {
							/* ignore */
						}
					})
	...
	function lastMatchTitle(points: number) {
		...
		return language.text(
			`Du fikk ${pointText(points)} på siste kamp`,
	...
	function progressEventScoreText(event: LeagueProgressEvent) {
		...
		if (event.etHome || event.etAway) s = `${event.etHome}–${event.etAway} ${language.text('e.e.o.', 'e.eo.', 'aet')}`;
		if (event.penHome || event.penAway) s += ` (${event.penHome}–${event.penAway} ${language.text('str', 'str', 'pens')})`;
		return s;
	}
	function progressEventMeta(event: LeagueProgressEvent) {
		const parts: string[] = [];
		if (!event.tipped) {
			parts.push(language.text('Ingen tips', 'Ingen tips', 'No tip'));
		} else if (event.exact) {
			parts.push(language.text('Eksakt', 'Eksakt', 'Exact'));
		} else {
			if (event.correctWinner) {
				parts.push(event.stage === 'group' ? language.text('Utfall', 'Utfall', 'Outcome') : language.text('Videre', 'Vidare', 'Through'));
			}
			if (event.correctGoalDiff) parts.push(language.text('Diff', 'Diff', 'Diff'));
			if (event.correctTotalGoals) parts.push(language.text('Mål', 'Mål', 'Goals'));
			if (!parts.length) parts.push(language.text('Ingen treff', 'Ingen treff', 'No hit'));
		}
		parts.push(language.text(`Totalt ${event.totalAfter} p`, `Totalt ${event.totalAfter} p`, `Total ${event.totalAfter} pts`));
		return parts.join(' · ');
	}
	...
	let nowHero = $derived.by<NowHero>(() => {
		...
		if (liveMatch) {
			return {
				...
				title: language.text(
					`${resultTeams(liveMatch)} spilles nå`,
					`${resultTeams(liveMatch)} spelast no`,
					`${resultTeams(liveMatch)} is playing now`
				),
				body: `${stageLabel(liveMatch)}${liveMatch.tvChannel ? ` · ${language.text('på TV', 'på TV', 'on TV')}` : ''}`,
				href: matchTipHref(liveMatch),
				label: language.text('Se kamp', 'Sjå kamp', 'View match'),
				match: liveMatch
			};
		}
		...
		return {
			...
			title: nextMatch
				? language.text(`Alt er levert. Neste kamp er ${resultTeams(nextMatch)}`, `Alt er levert. Neste kamp er ${resultTeams(nextMatch)}`, `All submitted. Next match is ${resultTeams(nextMatch)}`)
				: language.text('Alt er levert', 'Alt er levert', 'Everything is submitted'),
			body: nextMatch
	...
	let forecastPulse = $derived.by(() => {
		...
		return {
			kicker: fs.groupStageDone
				? language.text('VM-tips · sluttspill', 'VM-tips · sluttspel', 'World Cup tip · knockout')
				: language.text('VM-tips · gruppespill', 'VM-tips · gruppespel', 'World Cup tip · group stage'),
			title: champion
				? alive
					? language.text('Vinneren din er fortsatt med', 'Vinnaren din er framleis med', 'Your winner is still alive')
					: language.text('Vinneren din er ute', 'Vinnaren din er ute', 'Your winner is out')
				: language.text('VM-tipset ditt er i gang', 'VM-tipset ditt er i gang', 'Your World Cup tip is live'),
			body: champion
...
<header class="home-hero">
	...
	<div class="hero-chips">
		{#if leaguesLoaded}
			...
			{/if}
			{#each heroLeagues as { lg, rankNum, meRow } (lg.id)}
				{@const medal = rankNum === 1 ? '🥇' : rankNum === 2 ? '🥈' : rankNum === 3 ? '🥉' : `#${rankNum}`}
				<a href={`/leagues/${lg.id}`} class="hero-chip league-pill" class:mobile-hide={heroLeagues.length > 1 && lg.id !== activeLeague?.id}>
					<span>{lg.name}</span>
					{#if meRow && rankNum > 0 && rankNum !== 999999}<b>{medal}</b>{/if}
				</a>
			{/each}
		{/if}
		{#if activeLeagueRow}
	...
	{#if !tournamentFinished && tournamentStarted && activeLeague && leagueProgress && leagueProgress.events.length > 0}
		<section class="card tile progress-card home-span-support">
			...
			<div class="progress-summary">
				<span>
						<i>{language.text('Kamppoeng', 'Kamppoeng', 'Match points')}</i>
		...
		<section class="card tile standing-card home-span-support">
			<div class="hd">
				...
				<div class="league-card-actions">
					...
					{/if}
					<a class="hdlink" href={leagueHref}>{language.text('Hele tabellen', 'Heile tabellen', 'Full table')}</a>
				</div>
			</div>
