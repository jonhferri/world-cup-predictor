...
// Re-runs the tip backfill with corrected match status filter and firstPlayer
// handling. 0028 ran with wrong filters; this supersedes it.
func init() {
	m.Register(func(app core.App) error {
		...
		// firstPlayer stores a plain name string — no lookup needed.
		...
		upsertGroupTip := func(userID, home, away string, ftH, ftA int, firstPlayer, firstTeamName string) error {
			...
			tip.Set("firstPlayer", firstPlayer)
			return app.Save(tip)
		}
		...
		for _, tip := range allTips {
			playerName := tip.GetString("firstPlayer")
			player, err := app.FindFirstRecordByFilter("players", "name = {:n}", map[string]any{"n": playerName})
			if err != nil {
				continue // player not found in DB — skip
			}
